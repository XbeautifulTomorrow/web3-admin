      ...
      <el-table :data="revenueData" style="width: auto" height="400px" border>
        ...
        >
          <template slot-scope="scope">
            <span
              style="color: #d2d2d2"
              v-if="scope.row.lotteryStatus == 'RECLAIMED'"
              >已回收</span
            >
            <span
              style="color: #ff0707"
              v-if="scope.row.lotteryStatus == 'FAIL'"
              >交付失败</span
            >
            <span
              style="color: #09912e"
              v-if="scope.row.lotteryStatus == 'SUCCESS'"
              >已交付</span
            >
          </template>
        </el-table-column>
  ...
  methods: {
    ...
    searchFun() {
      ...
      if (transactionCompleteTime && transactionCompleteTime[0]) {
        finishStartTime = timeForStr(
          transactionCompleteTime[0],
          "YYYY-MM-DD HH:mm:ss"
        );
      }
      if (transactionCompleteTime && transactionCompleteTime[1]) {
        finishEndTime = timeForStr(
          transactionCompleteTime[1],
          "YYYY-MM-DD HH:mm:ss"
        );
      }
    ...
    async fetchOrderManagerList(isSearch = true) {
      ...
      delete data.size;
      delete data.page;
      const resAggregateQuery = await this.$http.getOrderManagerStatistics(
        data
      );
