  ...
  <div class="page-wrapper">
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <span
            ...
            @click="showWithdrawNft(1, scope.row.outWithdrawalNftList)"
            >查看</span
          >
          <span v-else>--</span>
        </template>
      ...
      >
        <template slot-scope="scope">
          <span
            v-if="scope.row.withdrawalType == 'NFT'"
            @click="showWithdrawNft(2, scope.row.arrivedWithdrawalnftlist)"
            >查看</span
          >
      ...
      >
        <template slot-scope="scope">
          ...
          <span
            style="color: #fd770a"
            v-if="scope.row.auditStatus == 'TRANSFERRING'"
            >转帐中</span
          >
    ...
    >
      <div class="nft-box">
        <div
          class="nft-item"
          v-for="(item, index) in withdrawNftList"
          :key="index"
        >
    ...
    >
      <div class="review-box">
            ...
            <div class="info-item-val">
              <div class="nft-box">
                <div
                  ...
                  reviewData.arrivedWithdrawalnftlist"
                  :key="index"
                >
                  <div style="min-width: 80px; height: 80px">
                    <el-image
        ...
        <div
          class="review-submit"
          v-if="reviewData && reviewData.auditStatus == 'WAIT'"
        >
...
export default {
  ...
  methods: {
    ...
    async fetchAssetWithdrawalList(isSearch = true) {
      ...
      delete data.size;
      delete data.page;
      const resAggregateQuery = await this.$http.getAssetWithdrawalStatistics(
        data
      );
    ...
    async withdrawNft(item, isExcute = false) {
      ...
      const salt = web3.utils.asciiToHex(dayjs(item.createTime).valueOf().toString());
      const delay = 1;
      // // 查询是否签名
